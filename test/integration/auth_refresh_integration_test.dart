...
void main() {
  group('Integration - Token Refresh', () {
    test(
      'refreshes expired access token and persists rotated bundle',
      () async {
    ...
    test('returns null when refresh token is missing', () async {
      ...
      final manager = TokenLifecycleManager(
        ...
        refreshExecutor: (_) async {
          fail(
            'refreshExecutor must not be called when refresh token is empty',
          );
        },
      );
    ...
    test(
      'a genuinely rejected refresh clears stored tokens and notifies the caller',
      () async {
        final tokenStorage = EncryptedTokenStorage(
          secretsManager: InMemorySecretsManager(),
        );
        ...
        var onRefreshFailedCalled = false;
        final manager = TokenLifecycleManager(
          tokenStorage: tokenStorage,
          refreshExecutor: (_) async => null,
