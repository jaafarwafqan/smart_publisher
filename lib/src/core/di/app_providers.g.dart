// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'app_providers.dart';

// **************************************************************************
// RiverpodGenerator
// **************************************************************************

String _$networkClientHash() => r'76ad1034bb7cd1d709a4663035d2a1cdadab184c';

/// See also [networkClient].
@ProviderFor(networkClient)
final networkClientProvider = Provider<NetworkClient>.internal(
  networkClient,
  name: r'networkClientProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$networkClientHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef NetworkClientRef = ProviderRef<NetworkClient>;
String _$secretsManagerHash() => r'5d501b893a3f8b964e89883630cd678ff089cf3f';

/// See also [secretsManager].
@ProviderFor(secretsManager)
final secretsManagerProvider = Provider<SecretsManager>.internal(
  secretsManager,
  name: r'secretsManagerProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$secretsManagerHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef SecretsManagerRef = ProviderRef<SecretsManager>;
String _$encryptionServiceHash() => r'88add38b0cfb7e6ebf269e504016ef291fc2492b';

/// See also [encryptionService].
@ProviderFor(encryptionService)
final encryptionServiceProvider = Provider<EncryptionService>.internal(
  encryptionService,
  name: r'encryptionServiceProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$encryptionServiceHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef EncryptionServiceRef = ProviderRef<EncryptionService>;
String _$secureTokenStorageHash() =>
    r'c2ca61bed7c920726d74cdcfe9788d79277c5ad7';

/// See also [secureTokenStorage].
@ProviderFor(secureTokenStorage)
final secureTokenStorageProvider = Provider<SecureTokenStorage>.internal(
  secureTokenStorage,
  name: r'secureTokenStorageProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$secureTokenStorageHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef SecureTokenStorageRef = ProviderRef<SecureTokenStorage>;
String _$oauthManagerHash() => r'551149f5497dbdfa4015a4669e182def36e6eca5';

/// See also [oauthManager].
@ProviderFor(oauthManager)
final oauthManagerProvider = Provider<OAuthManager>.internal(
  oauthManager,
  name: r'oauthManagerProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$oauthManagerHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef OauthManagerRef = ProviderRef<OAuthManager>;
String _$scopeAuthorizerHash() => r'505101e61605aa3c19de12e5021deb61aceeca39';

/// See also [scopeAuthorizer].
@ProviderFor(scopeAuthorizer)
final scopeAuthorizerProvider = Provider<ScopeAuthorizer>.internal(
  scopeAuthorizer,
  name: r'scopeAuthorizerProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$scopeAuthorizerHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef ScopeAuthorizerRef = ProviderRef<ScopeAuthorizer>;
String _$tokenLifecycleManagerHash() =>
    r'02d954fcaad341403d6eed2b2eecfdd80efa859d';

/// See also [tokenLifecycleManager].
@ProviderFor(tokenLifecycleManager)
final tokenLifecycleManagerProvider = Provider<TokenLifecycleManager>.internal(
  tokenLifecycleManager,
  name: r'tokenLifecycleManagerProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$tokenLifecycleManagerHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef TokenLifecycleManagerRef = ProviderRef<TokenLifecycleManager>;
String _$eventBusHash() => r'1ef0b5d71dac0388a8ddf8cde4b9a68e80476ae8';

/// See also [eventBus].
@ProviderFor(eventBus)
final eventBusProvider = Provider<EventBus>.internal(
  eventBus,
  name: r'eventBusProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$eventBusHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef EventBusRef = ProviderRef<EventBus>;
String _$eventDispatcherHash() => r'f4f658c9617ac6f88ac67ca20b3c6ba117cb0a90';

/// See also [eventDispatcher].
@ProviderFor(eventDispatcher)
final eventDispatcherProvider = Provider<EventDispatcher>.internal(
  eventDispatcher,
  name: r'eventDispatcherProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$eventDispatcherHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef EventDispatcherRef = ProviderRef<EventDispatcher>;
String _$authEventPublisherHash() =>
    r'f921fe51cd99d0a233ed4588032db1803c254750';

/// See also [authEventPublisher].
@ProviderFor(authEventPublisher)
final authEventPublisherProvider = Provider<AuthEventPublisher>.internal(
  authEventPublisher,
  name: r'authEventPublisherProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$authEventPublisherHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef AuthEventPublisherRef = ProviderRef<AuthEventPublisher>;
String _$draftStorageHash() => r'e08e8541e4e93ae9f0810f85a982965608d57d09';

/// See also [draftStorage].
@ProviderFor(draftStorage)
final draftStorageProvider = Provider<DraftStorage>.internal(
  draftStorage,
  name: r'draftStorageProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$draftStorageHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef DraftStorageRef = ProviderRef<DraftStorage>;
String _$outboxStoreHash() => r'1e1d4b706c384a2f15ab145decb23be5b309093b';

/// See also [outboxStore].
@ProviderFor(outboxStore)
final outboxStoreProvider = Provider<OutboxStore>.internal(
  outboxStore,
  name: r'outboxStoreProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$outboxStoreHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef OutboxStoreRef = ProviderRef<OutboxStore>;
String _$resumableUploadManagerHash() =>
    r'a28a14a3238145787197aa7744bdfb051f3076d9';

/// See also [resumableUploadManager].
@ProviderFor(resumableUploadManager)
final resumableUploadManagerProvider =
    Provider<ResumableUploadManager>.internal(
      resumableUploadManager,
      name: r'resumableUploadManagerProvider',
      debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
          ? null
          : _$resumableUploadManagerHash,
      dependencies: null,
      allTransitiveDependencies: null,
    );

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef ResumableUploadManagerRef = ProviderRef<ResumableUploadManager>;
String _$syncWorkerHash() => r'2213e733b22cc57a3a6555ab1847a2351d8134ac';

/// See also [syncWorker].
@ProviderFor(syncWorker)
final syncWorkerProvider = Provider<SyncWorker>.internal(
  syncWorker,
  name: r'syncWorkerProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$syncWorkerHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef SyncWorkerRef = ProviderRef<SyncWorker>;
String _$scheduleRepositoryHash() =>
    r'8d4d062b4aff8492043b51560fd390bb2d6074ac';

/// See also [scheduleRepository].
@ProviderFor(scheduleRepository)
final scheduleRepositoryProvider = Provider<ScheduleRepository>.internal(
  scheduleRepository,
  name: r'scheduleRepositoryProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$scheduleRepositoryHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef ScheduleRepositoryRef = ProviderRef<ScheduleRepository>;
String _$postRepositoryHash() => r'1537c3f82081775e46714df1e15e41e03da4931f';

/// See also [postRepository].
@ProviderFor(postRepository)
final postRepositoryProvider = Provider<PostRepository>.internal(
  postRepository,
  name: r'postRepositoryProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$postRepositoryHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef PostRepositoryRef = ProviderRef<PostRepository>;
String _$mediaRepositoryHash() => r'1c346df6393657cd9a0d8636106ede189cade8f9';

/// See also [mediaRepository].
@ProviderFor(mediaRepository)
final mediaRepositoryProvider = Provider<MediaRepository>.internal(
  mediaRepository,
  name: r'mediaRepositoryProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$mediaRepositoryHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef MediaRepositoryRef = ProviderRef<MediaRepository>;
String _$createPostUseCaseHash() => r'850bfb8b2fa9523cf5680c970df4d46ba29ca8d0';

/// See also [createPostUseCase].
@ProviderFor(createPostUseCase)
final createPostUseCaseProvider = Provider<CreatePost>.internal(
  createPostUseCase,
  name: r'createPostUseCaseProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$createPostUseCaseHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef CreatePostUseCaseRef = ProviderRef<CreatePost>;
String _$publishPostUseCaseHash() =>
    r'3f0621c9bf4908a5fa5be5dd7de9f66e156d6af0';

/// See also [publishPostUseCase].
@ProviderFor(publishPostUseCase)
final publishPostUseCaseProvider = Provider<PublishPost>.internal(
  publishPostUseCase,
  name: r'publishPostUseCaseProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$publishPostUseCaseHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef PublishPostUseCaseRef = ProviderRef<PublishPost>;
String _$schedulePostUseCaseHash() =>
    r'aed236f7fb8c0658dd734b66d33e8edc22f88438';

/// See also [schedulePostUseCase].
@ProviderFor(schedulePostUseCase)
final schedulePostUseCaseProvider = Provider<SchedulePost>.internal(
  schedulePostUseCase,
  name: r'schedulePostUseCaseProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$schedulePostUseCaseHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef SchedulePostUseCaseRef = ProviderRef<SchedulePost>;
String _$uploadMediaUseCaseHash() =>
    r'fb3e130b83a26c72f623d4ef9eba5df0d2c4678e';

/// See also [uploadMediaUseCase].
@ProviderFor(uploadMediaUseCase)
final uploadMediaUseCaseProvider = Provider<UploadMedia>.internal(
  uploadMediaUseCase,
  name: r'uploadMediaUseCaseProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$uploadMediaUseCaseHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef UploadMediaUseCaseRef = ProviderRef<UploadMedia>;
String _$compressMediaUseCaseHash() =>
    r'd7f6016da2643264fac94d4b9836049e1a8a0e60';

/// See also [compressMediaUseCase].
@ProviderFor(compressMediaUseCase)
final compressMediaUseCaseProvider = Provider<CompressMedia>.internal(
  compressMediaUseCase,
  name: r'compressMediaUseCaseProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$compressMediaUseCaseHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef CompressMediaUseCaseRef = ProviderRef<CompressMedia>;
String _$packageInfoHash() => r'f7bd90882137aac7a9f28e9d10eae6efeee8c9d0';

/// Sprint (Help Center, 2026-08-10): the single place the app name,
/// version, and build number are read from — `AboutSystemScreen` must
/// never hardcode these, since they already live in `pubspec.yaml` at
/// build time and this reads them back at runtime instead of duplicating
/// the value in a second place.
///
/// Copied from [packageInfo].
@ProviderFor(packageInfo)
final packageInfoProvider = FutureProvider<PackageInfo>.internal(
  packageInfo,
  name: r'packageInfoProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$packageInfoHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef PackageInfoRef = FutureProviderRef<PackageInfo>;
// ignore_for_file: type=lint
// ignore_for_file: subtype_of_sealed_class, invalid_use_of_internal_member, invalid_use_of_visible_for_testing_member, deprecated_member_use_from_same_package
